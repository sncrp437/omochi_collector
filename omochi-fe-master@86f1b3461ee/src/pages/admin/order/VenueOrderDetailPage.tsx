...
const VenueOrderDetailPage = () => {
  ...
  const changeStatus = async (status: StatusDisplayItem) => {
    ...
    if (status.status === OrderStatusEnum.Completed) {
      await handlePickupComplete();
      setIsLoadingChangeStatus(false);
      return;
    }
    try {
      await updateOrderStatus(orderDetail.id, {
  ...
  const handlePickupComplete = async () => {
    try {
      await confirmOrderPickup(orderDetail.id);
    } catch (error) {
...
export default VenueOrderDetailPage;
